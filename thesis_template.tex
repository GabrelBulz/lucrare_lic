...
Floods are between the most devastating natural disasters that our society has to face and they can produce tremendous assets and human lives loss. As a solution we tried to create a prediction model that can highlight the areas with high risk of being flooded during severe rainstorms.
\par 

One of our main goals was to create a cheap to produce and maintain application, that could be easy to use and understand even by untrained people.
\par 

This paper will offer some insides into the model skeleton and processing mechanics. The main idea behind our prediction model is that it will take as input satellite images over a land area and by combining the data it will firstly detect the water bodies using a special index called (normalized difference water index or NDWI) and then it will flood that area, starting from the detected water areas, based on the land topography.
